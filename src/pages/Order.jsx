...
function Order(){
    ...
    const handleSNameChange = (event) => { //for Restaurant input
        setSName(event.target.value);
        setIsDisabled(event.target.value === "" || Deli === "" || mName === ""); //เช็คว่าว่างมั้ย
      };
    
      const handleMNameChange = (event) => { //for Order input
        setMName(event.target.value);
        setIsDisabled(sName === "" || Deli === "" || event.target.value === "");
      };
      const handleDeliChange = (event) => { //for Delivery place input
        setDeli(event.target.value);
        setIsDisabled(sName === "" || mName === "" || event.target.value === "");
      };
      const handleSubmit = (event) => { //for submit in modal
